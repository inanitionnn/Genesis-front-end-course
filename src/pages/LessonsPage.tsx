...
import { useFetchCourseByIdQuery } from "../redux/services/coursesApi";
import { InfinitySpin } from "react-loader-spinner";
import { setCourseWithLessons } from "../redux/reducers/coursesSlice";
...
export default function LessonsPage() {
  ...
  const { data } = useFetchCourseByIdQuery(courseId || "");
  const coursesWithLessonsState = useAppSelector(
    (state) => state.courses.courseWithLessons
  );
  useEffect(() => {
    if (data) {
      if (!coursesWithLessonsState.find((course) => course.id === courseId)) {
        dispatch(setCourseWithLessons(data));
      }
    }
  }, [dispatch, data]);

  const course = coursesWithLessonsState.find(
  ...
  const images = [];
  if (course) {
    for (let order = 0; order < course.lessons.length; order++) {
      images.push(
        <LessonImage
          course={course}
  ...
  return (
    <>
      <Link
        to={`/`}
        className="mt-4 mb-8 mx-36 flex items-center justify-center"
      >
      ...
      <div className="container mx-auto">
        {!course ? (
          <div className="flex justify-center ml-52">
            <InfinitySpin width="400" color="#EA5F47" />
          </div>
        ) : (
          <div className="flex">
            <div className="w-4xl">
              ...
              {course.meta.courseVideoPreview && lessonOrder == -1 ? (
                <VideoPlayer videoUrl={course.meta.courseVideoPreview?.link} />
              ) : (
                <VideoPlayer videoUrl={course.lessons[lessonOrder].link} />
              )}
              <div className="text-xl font-medium my-2">
                {course.description}
              </div>
            </div>
            <div className="w-96 ml-12">{images}</div>
          </div>
        )}
      </div>
    </>
